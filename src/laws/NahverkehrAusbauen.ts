...
export default defineLaw({
  ...
  effects(data, startYear, currentYear): Partial<WritableBaseParams> {
    const relCapacity = (data.shortdistanceTransportCapacity / data.shortdistanceTransportUsage) * 100
    const potentialUsageIncrease = relCapacity >= 105 ? 0.01 * data.shortdistanceTransportUsage : 0
    ...
    const yearly: Partial<WritableBaseParams> = {
      ...
      shortdistanceTransportCapacity: 0.01 * data.shortdistanceTransportCapacity,
