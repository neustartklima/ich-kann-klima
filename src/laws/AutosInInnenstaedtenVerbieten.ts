...
export default defineLaw({
  ...
  effects(data, startYear, currentYear): Partial<WritableBaseParams> {
    ...
    if (data.shortdistanceTransportCapacity > data.shortdistanceTransportUsage * 1.2) {
    ...
    const potentialUsageIncrease = (startYear === currentYear ? 0.1 * data.shortdistanceTransportUsage : 0) as MioPsgrKm
  ...
  priority(game) {
    const relCapacity: Percent = (game.values.shortdistanceTransportCapacity / game.values.shortdistanceTransportUsage) * 100
