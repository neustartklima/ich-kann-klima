...
describe("EventMachine.getOne()", () => {
  it("should return the most probable event", () => {
    ...
    const found = [...Array(100).keys()].filter(() => getAnEvent(game, allEvents).title === "t1")
  ...
  it("should use randomly choose from equally probable events", () => {
    ...
    do {
      prev = current
      current = getAnEvent(game, allEvents).title
  ...
  it("should ignore impossible events", () => {
    ...
    const found = [...Array(100).keys()].some(() => getAnEvent(game, allEvents).title === "t1")
